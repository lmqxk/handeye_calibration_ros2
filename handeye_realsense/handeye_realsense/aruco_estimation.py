...
class ArucoNode(Node):
    def __init__(self):
        ...
        self.camera_calibration_parameters_filename = os.path.join(self._pkg_dir, config["camera_calibration_parameters_filename"])
        ...
        self.image_filename = os.path.join(self._pkg_dir, config["image_filename"])
        ...
        self.pose_count = 0
        self.last_corners = None   # 缓存最近检测到的 corners（供 keypress_callback 保存用）
        ...
        # 提前创建 OpenCV 窗口，让用户知道节点跑起来了
        cv2.namedWindow("camera", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("camera", 700, 500)
        placeholder = np.zeros((480, 640, 3), dtype=np.uint8)
        cv2.putText(placeholder, "等待图像...", (200, 240),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        cv2.imshow("camera", placeholder)
        cv2.waitKey(1)

        self.get_logger().info("OpenCV 窗口已创建，等待相机数据...")
    ...
    def listener_callback(self, data):
        ...
        if marker_ids is not None:
            ...
            if self._no_marker_count % 65 == 0:  # ~5s @ 13Hz
                self.get_logger().info(f"检测中... 未找到 ArUco 标记 ({len(rejected)} 个候选被拒绝)")
    ...
    def keypress_callback(self, msg: String):
        """监听 keypress_topic，支持 auto_calibrate 远程触发保存"""
        key = msg.data
        if key == 'q':
            if self.last_rvecs is not None and self.last_tvecs is not None:
                self.save_marker_data(self.last_rvecs, self.last_tvecs)
                if self.last_frame is not None:
                    self.save_image(self.last_frame)
                self.get_logger().info(f"auto_calibrate: 已保存 pose_{self.pose_count} marker 数据")
            else:
                self.get_logger().warning("auto_calibrate: 未检测到 ArUco 标记，跳过此帧！")
        elif key == 'e':
            self.get_logger().info("auto_calibrate: 结束程序...")
            rclpy.shutdown()
